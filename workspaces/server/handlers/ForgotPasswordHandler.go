...
func ForgotPasswordHandler() {
	...
	//todo
	//if email id entered exits in the users database then send email with otp
	enteredEmailID := "sheelaippili140@gmail.com"

	//getuserinfo
	if enteredEmailID == "sheelaippili140@gmail.com" {
		...
		//store the otp in the OTPverifies table

	} else {
		//todo
		//send json response that email could not be found to front-end
...
func CheckOtp() {
	...
	//Check if the otp entered for the email id is a vaolid otp and the right otp, if matched, then send response to front end for forgot password option
}
